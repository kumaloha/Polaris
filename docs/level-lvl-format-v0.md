...
- `personalization.persona_axes`：冷启动画像参数；用 `novelty/reward/challenge/strategy/cuteness/annoyance` 六轴控制生成与模拟偏好。
...
- `mechanism_specs`：机制库 v1 的本关子集；每个活跃目标/障碍/演员/奖励都必须声明状态动态、玩家动作、棋盘效果、兼容规则、求解器 hook 和 Godot 支持。
...
- `obstacle_composition.visual_grammar`：视觉构图契约；把“中心门/双区/边缘目标/环形围城”等审美意图变成焦点、密度、对称和轮廓检查。
...
```json
{
  ...
  "obstacle_composition": {
    ...
    "visual_grammar": {
      ...
      "density_band": [0.02, 0.20],
      "silhouette": "stardust_cluster",
      "anchor_layers": ["target_mark"],
...
14. `mechanism_specs` 必填，是机制库语言源；必须覆盖所有 active atoms，并声明 state_dynamics、player_action、board_effect、reward_effect、compatibility、intro_rule、mixing_rule、simulator_hook、godot_support。
15. `personalization.persona_axes` 必填，是冷启动个性化语言源；必须声明 novelty/reward/challenge/strategy/cuteness/annoyance 六轴。
16. `progression` 必填，是长期关卡节奏语言源；必须声明 episode、mechanic_lifecycle、reward_budget、annoyance_budget、difficulty_rhythm。
17. `obstacle_composition` 必填，是障碍构图语言源；必须声明 purpose、archetype、primary_blocker、action_vector、read_order、negative_space、delete_test、beauty_rules。
18. `obstacle_composition.visual_grammar` 必填，是视觉构图语言源；必须声明 focal_alignment、symmetry、density_band、silhouette、anchor_layers。
19. `level_design` 必填，是单关原则语言源；必须声明命题、主角机制、舞台、世界状态目标、情绪弧和负空间。
20. `director` 必填，并且要声明：主角机制、情绪弧、记忆点、负空间、四位一体、反堆料约束。
