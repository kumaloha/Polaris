...
| `special_gems` | `fx` | 部分 | 初盘特效需要 compiler 支持 `fx` 输出；当前 `level_library.gd` 会清空 `fx`，因此 v0 暂不保证 |
...
13. `director` 必填，并且要声明：主角机制、情绪弧、记忆点、负空间、四位一体、反堆料约束。
...
输入 `.lvl`，执行 lint/compile/structural v0 + `taste` gate，输出 validation JSON。`approved` 必须同时满足结构可玩和导演品味契约。
