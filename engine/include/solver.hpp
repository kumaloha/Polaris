...
inline bool has_legal_move(Grid& g, const std::vector<std::vector<int>>* coat,
    ...
    for (int y = 0; y < h; ++y)
        for (int x = 0; x < w; ++x) {
            if (x + 1 < w && is_legal_swap(g, {x, y}, {x + 1, y}, coat, choco)) return true;
            if (y + 1 < h && is_legal_swap(g, {x, y}, {x, y + 1}, coat, choco)) return true;
        }
    return false;
}
...
inline ChocoResolveResult resolve(Grid& g, const std::vector<int>& species, std::mt19937& rng,
    ...
    while (true) {
        ...
        if (choco) r.choco_cleared += eat_chocolate(*choco, matched);
...
enum ObjType { OBJ_SCORE, OBJ_COLLECT, OBJ_CLEAR_JELLY, OBJ_CLEAR_BLOCKER, OBJ_CLEAR_CHOCO };
