...
const SP_NONE := 0
const SP_LINE_H := 1     # 直线：清整行
const SP_LINE_V := 2     # 直线：清整列
...
static func _gravity_fixed_cell(grid: Array, coat: Array, choco: Array, row: int, col: int) -> bool:
	return grid[row][col] == WALL or (not coat.is_empty() and coat[row][col] > 0) or (not choco.is_empty() and choco[row][col] > 0)

static func _gravity_empty_cell(grid: Array, coat: Array, choco: Array, row: int, col: int) -> bool:
...
static func _gravity_movable_cell(grid: Array, coat: Array, choco: Array, row: int, col: int) -> bool:
	if row < 0 or row >= grid.size() or col < 0 or col >= grid[row].size():
		return false
	return grid[row][col] >= 0 and not _gravity_fixed_cell(grid, coat, choco, row, col)

static func _has_fall_obstacles(grid: Array, coat: Array, choco: Array) -> bool:
	for y in grid.size():
		for x in grid[y].size():
			if _gravity_fixed_cell(grid, coat, choco, y, x):
				return true
	return false

static func _gravity_cannon_cell(cannon: Array, row: int, col: int) -> bool:
	return not cannon.is_empty() and row >= 0 and row < cannon.size() and col >= 0 and col < cannon[row].size() and cannon[row][col] > 0

static func _gravity_blocked_above(grid: Array, coat: Array, choco: Array, cannon: Array, row: int, col: int) -> bool:
	if row <= 0 or col < 0 or grid.is_empty() or col >= grid[0].size():
		return false
	for y in range(row):
		if _gravity_cannon_cell(cannon, y, col):
			continue
		if _gravity_fixed_cell(grid, coat, choco, y, col):
			return true
	return false

static func _gravity_has_vertical_source_above(grid: Array, coat: Array, choco: Array, row: int, col: int) -> bool:
	if row <= 0 or col < 0 or grid.is_empty() or col >= grid[0].size():
		return false
	for y in range(row - 1, -1, -1):
		if _gravity_fixed_cell(grid, coat, choco, y, col):
			return false
		if _gravity_movable_cell(grid, coat, choco, y, col):
			return true
	return false

static func _move_gravity_cell(grid: Array, fx: Array, ing: Array, bomb: Array, popcorn: Array, mystery: Array, from_row: int, from_col: int, to_row: int, to_col: int) -> void:
	grid[to_row][to_col] = grid[from_row][from_col]
	grid[from_row][from_col] = EMPTY
	if not fx.is_empty():
		fx[to_row][to_col] = fx[from_row][from_col]
		fx[from_row][from_col] = SP_NONE
	if not ing.is_empty():
		ing[to_row][to_col] = ing[from_row][from_col]
		ing[from_row][from_col] = 0
	if not bomb.is_empty():
		bomb[to_row][to_col] = bomb[from_row][from_col]
		bomb[from_row][from_col] = 0
	if not popcorn.is_empty():
		popcorn[to_row][to_col] = popcorn[from_row][from_col]
		popcorn[from_row][from_col] = 0
	if not mystery.is_empty():
		mystery[to_row][to_col] = mystery[from_row][from_col]
		mystery[from_row][from_col] = 0

static func _try_fill_gravity_slot(grid: Array, fx: Array, coat: Array, choco: Array, cannon: Array, ing: Array, bomb: Array, popcorn: Array, mystery: Array, target_row: int, target_col: int) -> bool:
	if target_row <= 0 or not _gravity_empty_cell(grid, coat, choco, target_row, target_col):
		return false
	var source_row := target_row - 1
	var candidates := [Vector2i(target_col, source_row)]
	if _gravity_blocked_above(grid, coat, choco, cannon, target_row, target_col) and not _gravity_has_vertical_source_above(grid, coat, choco, target_row, target_col):
		candidates.append(Vector2i(target_col + 1, source_row))
		candidates.append(Vector2i(target_col - 1, source_row))
	for p in candidates:
		if _gravity_movable_cell(grid, coat, choco, p.y, p.x):
			_move_gravity_cell(grid, fx, ing, bomb, popcorn, mystery, p.y, p.x, target_row, target_col)
			return true
	return false

static func _clear_spawn_layers(fx: Array, layers: Dictionary, row: int, col: int) -> void:
	if not fx.is_empty():
		fx[row][col] = SP_NONE
	for key in ["ing", "bomb", "popcorn", "mystery"]:
		var layer: Array = layers.get(key, [])
	...
	if not up and _has_fall_obstacles(grid, coat, choco):
		...
		while moved and guard < max_steps:
			...
			for y in range(h - 1, 0, -1):
				for x in range(w):
					moved = _try_fill_gravity_slot(grid, fx, coat, choco, cannon, ing, bomb, popcorn, mystery, y, x) or moved
		return
	for x in w:
...
static func refill(grid: Array, species_set: Array, rng: RandomNumberGenerator, fx: Array = [], feed: Array = [], layers: Dictionary = {}) -> void:
	...
	if has_obstacles:
		...
		for _i in range(max_steps):
			...
			var spawned := false
			for x in range(w):
				if not _gravity_empty_cell(grid, coat, choco, 0, x):
					continue
				if has_feed:
...
static func _blast_cakes(grid: Array, cake: Array, cleared_set: Dictionary, already_hit: Dictionary = {}) -> Dictionary:
...
static func _resolve_plain(grid: Array, species_set: Array, rng: RandomNumberGenerator, feed: Array = [], do_refill: bool = true, cascades_out = null, layers: Dictionary = {}) -> Dictionary:
	...
	while true:
		...
		if has_cake:
			var cb := _blast_cakes(grid, cake, matched_set, cake_hit_this_resolve)
...
static func _resolve_fx(grid: Array, species_set: Array, rng: RandomNumberGenerator, fx: Array, feed: Array = [], do_refill: bool = true, cascades_out = null, layers: Dictionary = {}, preferred_spawn: Vector2i = Vector2i(-1, -1), preferred_line_kind: int = SP_NONE, force_preferred_spawn: bool = false) -> Dictionary:
	...
	while true:
		...
		if has_cake:
			var cb := _blast_cakes(grid, cake, cleared_set, cake_hit_this_resolve)
			cake_destroyed += cb["destroyed"]
...
static func classify_matches(grid: Array, layers: Dictionary = {}, preferred_spawn: Vector2i = Vector2i(-1, -1), preferred_line_kind: int = SP_NONE, force_preferred_spawn: bool = false) -> Dictionary:
	...
	for r in (h_runs + v_runs):
		var spawn_pos: Vector2i = _preferred_spawn_pos(r["cells"], r["mid"], preferred_spawn)
		if r["len"] >= 5 and not spawn_at.has(spawn_pos):
			spawns.append({"pos": spawn_pos, "kind": SP_COLORBOMB})
			spawn_at[spawn_pos] = true

	for c in matched.keys():
		var spawn_pos: Vector2i = _intersection_spawn_pos(c, h_runs, v_runs, preferred_spawn)
		if in_h.has(c) and in_v.has(c) and not spawn_at.has(spawn_pos):
			spawns.append({"pos": spawn_pos, "kind": SP_BOMB})  # T/L/+：真实交换时落在移动棋子的新位置，否则落交点
			spawn_at[spawn_pos] = true

	# 4 连生成的条纹方向与匹配轴互补：横向 4 连生成竖条，纵向 4 连生成横条。
	# SP_LINE_H / SP_LINE_V 仍分别表示触发时清整行 / 清整列。
	for r in h_runs:
		var spawn_pos: Vector2i = _preferred_spawn_pos(r["cells"], r["mid"], preferred_spawn)
		if r["len"] == 4 and not _run_intersects(r["cells"], in_v) and not spawn_at.has(spawn_pos):
			spawns.append({"pos": spawn_pos, "kind": SP_LINE_V})
			spawn_at[spawn_pos] = true
	for r in v_runs:
		var spawn_pos: Vector2i = _preferred_spawn_pos(r["cells"], r["mid"], preferred_spawn)
		if r["len"] == 4 and not _run_intersects(r["cells"], in_h) and not spawn_at.has(spawn_pos):
			spawns.append({"pos": spawn_pos, "kind": SP_LINE_H})
			spawn_at[spawn_pos] = true
...
static func _preferred_spawn_pos(cells: Array, fallback: Vector2i, preferred_spawn: Vector2i) -> Vector2i:
	if preferred_spawn.x >= 0 and cells.has(preferred_spawn):
...
static func _intersection_spawn_pos(intersection: Vector2i, h_runs: Array, v_runs: Array, preferred_spawn: Vector2i) -> Vector2i:
	if preferred_spawn.x < 0:
		return intersection
	for h_run in h_runs:
		var h_cells: Array = h_run["cells"]
		if not h_cells.has(intersection):
			continue
		for v_run in v_runs:
			var v_cells: Array = v_run["cells"]
			if v_cells.has(intersection) and (h_cells.has(preferred_spawn) or v_cells.has(preferred_spawn)):
				return preferred_spawn
	return intersection
...
static func account_clears(grid: Array, cells: Array, fx: Array = [], rng: RandomNumberGenerator = null, species_set: Array = [], layers: Dictionary = {}) -> Dictionary:
	...
	var has_ing := not ing.is_empty()
	var locked := {}  # 开始时锁住/巧克力/原料/爆米花/神秘糖的格：只破层啃食/命中/揭开，不被清/不计入收集·果冻
	var cleared_set := {}
	for p in cells:
		cleared_set[p] = true
	if has_coat:
	...
	if has_ing:
		for p in cells:
			if ing[p.y][p.x] > 0:
				locked[p] = true   # 原料格不被彩球/融合/同类等直清清空，保持与普通 resolve 路径一致
	...
	for pos in cells:
		if locked.has(pos):
			continue  # 锁住/巧克力/原料/爆米花/神秘糖格不被清除，不计入收集/果冻（仅上面破层/啃食/命中/揭开）
		var sp_p: int = grid[pos.y][pos.x]
		if sp_p >= 0:
			by_species[sp_p] = by_species.get(sp_p, 0) + 1
		if has_jelly and jelly[pos.y][pos.x] > 0:
	...
	if has_cake:
		...
		for bp in cb["blast"]:
			...
			if (has_coat and coat[bp.y][bp.x] > 0) or (has_choco and choco[bp.y][bp.x] > 0):
				continue
			if has_ing and ing[bp.y][bp.x] > 0:
				continue
			if has_pop and popcorn[bp.y][bp.x] > 0:
				# 蛋糕引爆/大爆炸波及的爆米花格 → 当作特效命中：popcorn-1、归0变彩球(SP_COLORBOMB)、不清空。
...
static func _normalize_layers_arg(layers_or_coat = {}) -> Dictionary:
	if layers_or_coat is Dictionary:
		return layers_or_coat
	return {"coat": layers_or_coat}

# 枚举全部合法交换（预知/默认提示用）。返回 [[a:Vector2i, b:Vector2i], ...]，完整障碍层感知。
static func legal_moves(grid: Array, layers_or_coat = {}) -> Array:
	var layers := _normalize_layers_arg(layers_or_coat)
	var h := grid.size()
	if h == 0:
		return []
	var w: int = grid[0].size()
...
static func best_moves(grid: Array, k: int, layers_or_coat = {}, objectives: Array = []) -> Array:
	var layers := _normalize_layers_arg(layers_or_coat)
	var moves := legal_moves(grid, layers)
	var scored := []
	for mv in moves:
		_swap_cells(grid, mv[0], mv[1])
		var m: Array = find_matches(grid, layers)
		var val := m.size()
