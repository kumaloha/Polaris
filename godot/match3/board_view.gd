...
func _wall_slide_position_at_table(start_pos: Vector2, points: Array, table: Array, progress: float) -> Vector2:
	return LevelMotion.wall_slide_position_at_table(start_pos, points, table, progress)
...
func _tween_wall_slide_node(node: Sprite2D, target: Vector2, cell_path: Array = [], duration_override: float = -1.0) -> float:
	...
	var table := LevelMotion.wall_slide_arclength_table(start_pos, points)
	...
	var apply_position := func(progress: float) -> void:
		if node != null and is_instance_valid(node):
			node.position = _wall_slide_position_at_table(start_pos, points, table, progress)
