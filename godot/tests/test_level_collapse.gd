...
func test_level_collapse_refill_repairs_missing_visual_gems() -> void:
	var scene: PackedScene = load("res://Level.tscn")
	var level := scene.instantiate()
	assert_true(level.has_method("_repair_missing_gem_nodes_from_board"), "Level exposes a targeted visual hole repair helper")
	level.free()
	var src := FileAccess.get_file_as_string("res://match3/level.gd")
	var start: int = src.find("func _collapse_and_refill")
	assert_true(start >= 0, "_collapse_and_refill exists")
...
func test_level_wall_collapse_uses_cross_column_slide_visuals() -> void:
	var scene: PackedScene = load("res://Level.tscn")
	var level := scene.instantiate()
	assert_true(level.has_method("_sync_wall_slide_visuals"), "Level has a wall-aware cross-column visual collapse helper")
	assert_true(level.has_method("_grid_has_fall_obstacle"), "Level detects all gravity-blocking cells, not only stone walls")
	...
	assert_false(level.call("_grid_has_fall_obstacle", plain), "a plain board has no fall obstacle")
	var walled := [[0, ME.WALL, 2], [1, 2, 0], [2, 0, 1]]
	assert_true(level.call("_grid_has_fall_obstacle", walled), "a board with a wall is detected as having a fall obstacle")
	level.free()
...
func test_level_wall_slide_visuals_tween_cell_steps() -> void:
	...
	assert_true(level.has_method("_wall_slide_path_points"), "Level exposes a path builder for wall slide visuals")
	assert_true(level.has_method("_wall_slide_position_at"), "Level samples wall slide paths continuously")
	assert_true(level.has_method("_tween_wall_slide_node"), "Level tweens wall slide nodes through path steps")
	...
	var pts: Array = level.call("_wall_slide_path_points", sp, tp)
	var at0: Vector2 = level.call("_wall_slide_position_at", sp, pts, 0.0)
	var at1: Vector2 = level.call("_wall_slide_position_at", sp, pts, 1.0)
...
func test_level_wall_slide_visuals_only_cross_columns_under_fall_obstacles() -> void:
	var scene: PackedScene = load("res://Level.tscn")
	var level := scene.instantiate()
	assert_true(level.has_method("_wall_slide_target_has_fall_obstacle_above"), "Level can tell whether a target is under a gravity-blocking obstacle")
	level.free()
...
func test_opening_boss_casts_stones_only_and_ice_falls_with_board() -> void:
	...
	# 钉源码理由: 石头开局收集墙格(_opening_wall_cells)、用 boss 光束色(OPENING_STONE_COLOR)、光束后才出石头标记, 锁住 boss 施石顺序
