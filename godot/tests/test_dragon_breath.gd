...
func test_youth_dragon_flight_lifts_body_to_board_center_without_rescaling() -> void:
	...
	var pre_rise := float(timing.get("pre_rise", -1.0))
	var rise := float(timing.get("rise", 0.0))
	var hold := float(timing.get("hold", 0.0))
	var fall := float(timing.get("fall", 0.0))
	var post_fall := float(timing.get("post_fall", -1.0))
	assert_true(offset.y < -120.0, "youth dragon takes off upward from the skill bar")
	assert_true(absf(offset.x) <= 0.001, "youth dragon flight only changes Y; X stays anchored to the original slot")
	assert_true(absf(airborne_center.y - board_center.y) <= 1.0, "youth dragon body reaches the board middle height")
	assert_true(absf(airborne_center.x - start_center.x) <= 0.001, "board-middle flight target preserves the original X position")
