...
func test_level_scene_renders_blockers_as_keyed_barrier_ice_sprites() -> void:
	...
	if marker is Sprite2D:
		...
		assert_true(drawn_size.x <= level.cell_size * 0.92 and drawn_size.y <= level.cell_size * 0.92, "barrier visible art stays inside one board cell without overlapping neighboring gems")
	level.free()
