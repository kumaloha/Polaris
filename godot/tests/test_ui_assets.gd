...
const COLORBOMB_CORE_SOURCE := "resources/0.02/gem/55.png"
const COLORBOMB_CORE_SYNCED := "res://assets/level/colorbomb_55.png"
...
func test_colorbomb_core_uses_synced_55_art() -> void:
	assert_true(FileAccess.file_exists(_repo_path(COLORBOMB_CORE_SOURCE)), "source 55.png 5-match art exists")
	assert_true(FileAccess.file_exists(COLORBOMB_CORE_SYNCED), "synced 55.png 5-match art exists")
	var src := FileAccess.get_file_as_string("res://match3/level.gd")
	assert_true(src.contains('const COLORBOMB_CORE := "%s"' % COLORBOMB_CORE_SYNCED), "5-match colorbomb core uses synced 55.png")
...
func test_colorbomb_core_keeps_cell_sized_fit_after_art_swap() -> void:
	...
	assert_true(tex != null, "55.png 5-match art loads as a texture")
	if tex != null:
