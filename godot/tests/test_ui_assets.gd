...
const COLORBOMB_CORE_SOURCE := "resources/0.02/gem/gold_amulet.png"
const COLORBOMB_CORE_SYNCED := "res://assets/level/colorbomb_gold_amulet.png"
...
func test_colorbomb_core_uses_synced_gold_amulet_art() -> void:
	assert_true(FileAccess.file_exists(_repo_path(COLORBOMB_CORE_SOURCE)), "source gold_amulet.png 5-match art exists")
	assert_true(FileAccess.file_exists(COLORBOMB_CORE_SYNCED), "synced gold_amulet.png 5-match art exists")
	var src := FileAccess.get_file_as_string("res://match3/level.gd")
	assert_true(src.contains('const COLORBOMB_CORE := "%s"' % COLORBOMB_CORE_SYNCED), "5-match colorbomb core uses synced gold_amulet.png")
	var img := Image.load_from_file(ProjectSettings.globalize_path(COLORBOMB_CORE_SYNCED))
	assert_true(img != null and img.detect_alpha() != Image.ALPHA_NONE, "synced gold_amulet.png keeps transparency so it does not render as a square")
...
func test_colorbomb_core_keeps_cell_sized_fit_after_art_swap() -> void:
	...
	assert_true(tex != null, "gold_amulet.png 5-match art loads as a texture")
	if tex != null:
