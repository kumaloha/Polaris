...
const COLORBOMB_CORE_SOURCE := "resources/0.02/gem/pp.png"
const COLORBOMB_CORE_SYNCED := "res://assets/level/colorbomb_pp.png"
...
func test_colorbomb_core_uses_synced_pp_art() -> void:
	assert_true(FileAccess.file_exists(_repo_path(COLORBOMB_CORE_SOURCE)), "source pp.png 5-match art exists")
	assert_true(FileAccess.file_exists(COLORBOMB_CORE_SYNCED), "synced pp.png 5-match art exists")
	var src := FileAccess.get_file_as_string("res://match3/level.gd")
	assert_true(src.contains('const COLORBOMB_CORE := "%s"' % COLORBOMB_CORE_SYNCED), "5-match colorbomb core uses synced pp.png")
	var img := Image.load_from_file(ProjectSettings.globalize_path(COLORBOMB_CORE_SYNCED))
	assert_true(img != null and img.detect_alpha() != Image.ALPHA_NONE, "synced pp.png is keyed to transparency so it does not render as a white square")
...
func test_colorbomb_core_keeps_cell_sized_fit_after_art_swap() -> void:
	...
	assert_true(tex != null, "pp.png 5-match art loads as a texture")
	if tex != null:
